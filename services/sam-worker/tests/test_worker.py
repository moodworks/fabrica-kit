...
class DirectRuntimeTests(unittest.TestCase):
    ...
    def test_production_staging_classification_rejects_partial_artifacts(self) -> None:
        ...
        def state_for(staged: tuple[bool, bool, bool]) -> str:
            with (
                patch("sam_worker.runtime.MANIFEST_PATH", Artifact(staged[0])),
                patch("sam_worker.runtime.CONFIG_PATH", Artifact(staged[1])),
                patch("sam_worker.runtime.CHECKPOINT_PATH", Artifact(staged[2])),
                patch(
                    "sam_worker.runtime.ProductionSamEngine",
                    return_value=FakeEngine([]),
                ),
            ):
                return create_production_runtime().readiness_state()

        self.assertEqual(state_for((False, False, False)), MODEL_NOT_STAGED)
        self.assertEqual(state_for((True, True, True)), MODEL_STAGED_NOT_LOADED)
        for staged in (
            (True, False, False),
            (False, True, False),
            (False, False, True),
